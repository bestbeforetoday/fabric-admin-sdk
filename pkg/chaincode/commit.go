...
func CreateCommitProposal(Definition Definition, id identity.SigningIdentity) (*peer.Proposal, error) {
	...
	return proposal.NewProposal(id, lifecycleChaincodeName, commitTransactionName, proposal.WithChannel(Definition.ChannelID), proposal.WithArguments(argsBytes))
}
