...
func createReadinessCheckProposal(Definition Definition, id identity.Identity) (*peer.Proposal, error) {
	...
	return proposal.NewProposal(id, lifecycleChaincodeName, checkCommitReadinessTransactionName, proposal.WithChannel(Definition.ChannelID), proposal.WithArguments(argsBytes))
}
